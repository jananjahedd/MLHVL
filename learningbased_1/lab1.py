...
def mnist_train_test():
    ...
    Returns:
        The x_train, y_train, x_test and y_test test tuples loaded with MNIST data
    """
...
def linear_activation(x_train, y_train, x_test, y_test):
    """
    Function that applies linear activation (default) to the model (fully connected network)
...
def relu_activation(x_train, y_train, x_test, y_test):
    """
    Function that applies rectified (relu) activation to the model (fully connected network)
...
def deep_conv_networks(x_train, y_train, x_test, y_test):
    ...
    # flatenning 3 spatial dimensions - reshape to 60000,28,28,1 and 10000,28,28,1
    ...
    # compile model
    model.compile(loss='categorical_crossentropy', optimizer=keras.optimizers.Adadelta(learning_rate=float(1)), metrics=['accuracy'])
...
def plots(history):
    """
    Function in charge of generating all the three different plots needed in this first part
...
def evaluate_model(model, xtest, ytest):
    ...
    Returns:
        loss, accuracy: loss and accuracy of the model's performance based on test set
    """
    loss, accuracy = model.evaluate(xtest, ytest, verbose=0)
    print(f"Evaluation linear activation model: {loss} {accuracy}")
